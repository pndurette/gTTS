...
class gTTS:
        ...
        for idx, part in enumerate(self.text_parts):
            ...
            headers = {
                ...
                "User-Agent": "Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 "
                              "(KHTML, like Gecko) Chrome/60.0.3112.113 Mobile Safari/537.36"
            }
            if self.debug:
                print(payload)
            try:
    ...
    def set_language(self, lang):
        if not lang or lang.lower() not in self.LANGUAGES:
            raise NameError("The input language is not valid.")
        self.lang = lang.lower()

    def say(self, text=None, clean=True):
        """
        Streams speech from text to normal audio stream
        :param text: if not given, self.text will be used
        :param clean: remove temporary file flag
        :return:
        """
        if text:
            self.text = text
